...
public enum ErrorCode {
    ...
    CONTRACT_DATE_INVALID("CONTRACT_DATE_INVALID", "合約起訖日期設定錯誤"),

    // 採購
    PROCUREMENT_NOT_FOUND("PROCUREMENT_NOT_FOUND", "採購單不存在"),
    PROCUREMENT_STATUS_INVALID("PROCUREMENT_STATUS_INVALID", "採購單狀態不允許此操作"),
    PROCUREMENT_PRESET_NOT_FOUND("PROCUREMENT_PRESET_NOT_FOUND", "找不到該採購預設方案"),

    // 太陽能
    SOLAR_DEVICE_NOT_FOUND("SOLAR_DEVICE_NOT_FOUND", "太陽能設備不存在"),
    SOLAR_RECORD_DUPLICATE("SOLAR_RECORD_DUPLICATE", "該設備當月發電紀錄已存在"),
    SOLAR_DEVICE_ALREADY_INACTIVE("SOLAR_DEVICE_ALREADY_INACTIVE", "設備已停用，無法變更狀態"),
    SOLAR_RECORD_NOT_FOUND("SOLAR_RECORD_NOT_FOUND", "月發電紀錄不存在"),

    // 用電量
    ELECTRICITY_RECORD_NOT_FOUND("ELECTRICITY_RECORD_NOT_FOUND", "找不到該用電量記錄"),
